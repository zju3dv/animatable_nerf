...
def ppts_to_tpose(net, pose_pts, batch):
    ...
    init_pbw, pnorm = sample_utils.sample_blend_closest_points(pose_pts, batch['pvertices'], batch['weights'])
    ...
    init_tbw, tnorm = sample_utils.sample_blend_closest_points(tpose, batch['tvertices'], batch['weights'])
...
def tpose_to_ppts(net, tpose, batch):
    ...
    tpose = pose_points_to_tpose_points(tpose, tbw, batch['big_A'])
