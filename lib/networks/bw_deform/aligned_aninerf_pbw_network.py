...
class Network(nn.Module):
    ...
    def calculate_neural_blend_weights(self, pose_pts, smpl_bw, poses):
        pts = embedder.xyz_embedder(pose_pts)
        pts = pts.transpose(1, 2)
        latent = poses
        latent = latent[..., None].expand(*latent.shape, pts.size(2))
        features = torch.cat((pts, latent), dim=1)
    ...
    def pose_points_to_tpose_points(self, pose_pts, pose_dirs, batch):
        ...
        pbw = self.calculate_neural_blend_weights(pose_pts, init_pbw, batch['poses'])
    ...
    def forward(self, wpts, viewdir, dists, batch):
        ...
        zero_pose = torch.zeros_like(batch['poses'])
        tbw = self.calculate_neural_blend_weights(tpose, init_tbw, zero_pose)
