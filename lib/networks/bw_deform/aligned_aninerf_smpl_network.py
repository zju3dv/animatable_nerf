...
class Network(nn.Module):
    ...
    def pose_points_to_tpose_points(self, pose_pts, pose_dirs, batch):
        ...
        return tpose, tpose_dirs, init_bigpose
    ...
    def forward(self, wpts, viewdir, dists, batch):
        ...
        tpose, tpose_dirs, init_bigpose = self.pose_points_to_tpose_points(
    ...
    def get_alpha(self, wpts, batch):
        ...
        nerf_nn_output = self.tpose_human.nerf_network(tpose, batch)
...
class TPoseHuman(nn.Module):
    ...
    def forward(self, wpts, viewdir, dists, batch):
        # calculate nerf
        nerf_nn_output = self.nerf_network(wpts, batch)
        alpha = nerf_nn_output[:, 0]
